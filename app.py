...
# if the network fails mid-demo, the app falls back to the last good data instead of crashing.
...
# ---------------- DATA LAYER (hardened) ----------------
# These functions cache every successful download to disk, one file per ticker.
# In normal use they fetch live data; if a fetch fails they fall back to the cache so
# the app never crashes on a network hiccup. Set demo_mode=True to read cache only.
...
def get_stock_data(tickers, period="1y", demo_mode=False):
    ...
    if demo_mode:
        try:
            ...
            pass  # cache missing -> try live below instead of failing
    ...
    try:
        ...
        _save_prices_to_cache(raw, period)        # refresh cache on every good fetch
        return raw
    except Exception:
        return _load_prices_from_cache(tickers, period)  # network failed -> use last good data


@st.cache_data(ttl=3600, show_spinner=False)
def get_dividend_data(tickers, prices=None, demo_mode=False):
    # Computes dividend yield WITHOUT the slow/flaky Ticker.info call:
    # last price comes from the already-downloaded price table.
    tickers = list(tickers)
    rows = []
...
def recommended_portfolio(allocation, equity_tickers):
    # Turns the strategic allocation (Equity/Bonds/Cash %) into a concrete ticker->weight
    # map, so the analytics can run on the SAME portfolio the advisor recommends.
    sleeves = {
...
def portfolio_metrics(prices, weights, risk_free_rate=0.0):
    ...
    # Sharpe ratio now correctly nets out the risk-free rate.
...
def calculate_portfolio_beta(prices, weights, period, demo_mode=False):
    ...
    # Pick the benchmark column robustly whether or not SPY is also in the portfolio.
...
def hedge_portfolio_analysis(prices, weights, portfolio_value, period, risk_free_rate=0.0, demo_mode=False):
    ...
    # Only add hedge assets that aren't already in the portfolio (avoids a join error).
...
def enhanced_ai_recommendation(
    age, risk_tolerance, horizon, score, expected_return, volatility, sharpe,
    allocation, macro_label, dividend_yield, salary_growth_rate, contribution_growth_rate
):
    recs = []
...
def generate_ai_advisor_note(api_key, profile, metrics, allocation, macro):
    # OPTIONAL: real LLM advisor. Only runs if an API key is set. On any problem it returns
    # a short note so the rule-based summary below still carries the demo.
...
# Optional Anthropic key (for the AI advisor note). Safe if it isn't set.
try:
...
try:
    ...
    # Build the active portfolio: either the typed tickers, or the recommended allocation.
    ...
    try:
        ...
        dividend_df = pd.DataFrame({
            "Ticker": tickers, "Last Annual Dividend": 0,
            "Estimated Price": np.nan, "Dividend Yield": 0
        })
    ...
    with tab_plan:
        ...
        with col2:
            fig_alloc = px.pie(
                allocation_df, names="Asset Class", values="Allocation (%)",
                hole=0.55, title="Recommended Allocation", color="Asset Class",
        ...
        projection = wealth_projection_with_growth(
            ...
            annual_return=assumed_return, years=horizon,
            contribution_growth_rate=contribution_growth_rate
        )

        income_schedule = income_growth_schedule(
            annual_income=annual_income, monthly_savings=monthly_savings,
            salary_growth_rate=salary_growth_rate,
            contribution_growth_rate=contribution_growth_rate, years=horizon
        )
        ...
        fig_projection = px.area(
            projection, x="Year", y="Projected Value",
        ...
        fig_contribution.add_trace(go.Scatter(
            x=income_schedule["Year"], y=income_schedule["Projected Salary"],
            mode="lines+markers", name="Projected Salary", line=dict(width=4, color="#0EA5E9")
        ))
        fig_contribution.add_trace(go.Scatter(
            x=income_schedule["Year"], y=income_schedule["Annual Contribution"],
            mode="lines+markers", name="Annual Contribution", line=dict(width=4, color="#10B981")
        ))
    ...
    with tab_invest:
        ...
        <div class="feature-card">
            ...
            <div class="card-text">Track selected stocks and ETFs against a broad market benchmark.</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        for i, ticker in enumerate(normalized.columns):
            fig_prices.add_trace(go.Scatter(
                ...
                name=ticker, line=dict(width=3, color=colors[i % len(colors)])
            ))
        ...
        try:
            ...
            fig_prices.add_trace(go.Scatter(
                x=benchmark_norm.index, y=benchmark_norm["SPY"], mode="lines",
                name="Benchmark: SPY", line=dict(width=3, dash="dash", color="#111827")
            ))
        except Exception:
    ...
    with tab_income:
        ...
        <div class="feature-card">
            ...
            <div class="card-text">Estimate dividend yield, annual income, dividend growth, and the long-term impact of reinvesting dividends.</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        display_dividend_df["Estimated Price"] = display_dividend_df["Estimated Price"].apply(lambda x: f"${x:.2f}" if pd.notna(x) else "N/A")
        ...
        fig_div_yield = px.bar(
            dividend_df, x="Ticker", y="Dividend Yield", title="Estimated Dividend Yield by Asset",
            color="Ticker", color_discrete_sequence=["#10B981", "#0EA5E9", "#8B5CF6", "#F59E0B", "#EF4444", "#14B8A6", "#6366F1"]
        )
        ...
        dividend_projection = dividend_reinvestment_projection(
            starting_value=current_savings, monthly_savings=monthly_savings,
            expected_return=expected_return, dividend_yield=portfolio_dividend_yield,
        ...
        fig_reinvest.add_trace(go.Scatter(
            x=dividend_projection["Year"], y=dividend_projection["With Dividend Reinvestment"],
            mode="lines", name="With Dividend Reinvestment", line=dict(width=4, color="#10B981")
        ))
        fig_reinvest.add_trace(go.Scatter(
            x=dividend_projection["Year"], y=dividend_projection["Without Dividend Reinvestment"],
            mode="lines", name="Without Dividend Reinvestment", line=dict(width=4, dash="dash", color="#0EA5E9")
        ))
        ...
        fig_income_growth = px.area(
            dividend_projection, x="Year", y="Estimated Annual Dividend Income",
            title="Projected Annual Dividend Income Growth", color_discrete_sequence=["#F59E0B"]
        )
    ...
    with tab_analyze:
        ...
        <div class="feature-card">
            ...
            <div class="card-text">Analyze return, volatility, Sharpe ratio, diversification, and correlations using the selected portfolio weights.</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if len(tickers) >= 2:
            corr = returns.corr()
            fig_corr = px.imshow(
                corr, text_auto=True, title="Asset Correlation Matrix",
    ...
    with tab_optimize:
        ...
        <div class="feature-card">
            ...
            <div class="card-text">Run Monte Carlo simulations and find portfolios with stronger risk-adjusted performance.</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if len(tickers) < 2:
            ...
            fig_frontier = px.scatter(
                ...
                title="Efficient Frontier Simulation", color_continuous_scale="Viridis"
            )
            fig_frontier.add_trace(go.Scatter(
                x=[best_portfolio["Volatility"]], y=[best_portfolio["Return"]], mode="markers",
                marker=dict(size=18, symbol="star", color="#F59E0B"), name="Max Sharpe"
            ))
            fig_frontier.add_trace(go.Scatter(
                x=[min_vol_portfolio["Volatility"]], y=[min_vol_portfolio["Return"]], mode="markers",
                marker=dict(size=15, symbol="diamond", color="#0EA5E9"), name="Min Volatility"
            ))
        ...
        if st.button("Run Standard Monte Carlo Simulation"):
            mc = monte_carlo_simulation(
                start_value=current_savings, monthly_savings=monthly_savings,
                expected_return=expected_return, volatility=volatility, years=horizon,
                contribution_growth_rate=contribution_growth_rate, simulations=500
            )
    ...
    with tab_macro:
        ...
        <div class="feature-card">
            ...
            <div class="card-text">Stress-test the portfolio under policy uncertainty, trade uncertainty, geopolitical risk, and recession-style scenarios.</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown(f"""
        <div class="risk-box"><b>Scenario explanation:</b> {scenario_data["explanation"]}</div>
        """, unsafe_allow_html=True)
        ...
        fig_macro = px.bar(
            macro_inputs_df, x="Indicator", y="Input Value", color="Indicator",
            title="Macro Uncertainty Indicators", color_discrete_sequence=["#10B981", "#0EA5E9", "#EF4444"]
        )
        ...
        if st.button("Run Macro-Adjusted Monte Carlo Simulation"):
            macro_mc = monte_carlo_simulation(
                start_value=current_savings, monthly_savings=monthly_savings,
                expected_return=adjusted_return, volatility=adjusted_volatility, years=horizon,
                contribution_growth_rate=contribution_growth_rate, simulations=500
            )
    ...
    with tab_protect:
        ...
        <div class="feature-card">
            ...
            <div class="card-text">Estimate beta, downside risk, hedged allocation, and stress-test portfolio performance.</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        try:
            ...
            if hedge_result is not None:
                ...
                st.markdown("""
                <div class="accent-box">The hedge simulation shifts 20% of the portfolio into defensive assets (BND and GLD).</div>
                """, unsafe_allow_html=True)
                st.dataframe(hedge_comparison, use_container_width=True)

                fig_hedge = px.bar(
                    hedge_weights_df, x="Asset", y="Hedged Weight (%)", title="Suggested Hedged Allocation",
                    color="Asset", color_discrete_sequence=["#10B981", "#0EA5E9", "#8B5CF6", "#F59E0B", "#EF4444", "#14B8A6"]
                )
    ...
    <div class="feature-card">
        ...
        <div class="card-text">
            A recommendation engine explains risk profile, contribution growth, dividend potential,
    ...
    ai_recs = enhanced_ai_recommendation(
        age=age, risk_tolerance=risk_tolerance, horizon=horizon, score=score,
        expected_return=expected_return, volatility=volatility, sharpe=sharpe,
        allocation=allocation, macro_label=macro_label, dividend_yield=portfolio_dividend_yield,
        salary_growth_rate=salary_growth_rate, contribution_growth_rate=contribution_growth_rate
    )

    # If an Anthropic API key is configured, offer a natural-language AI note.
    # Without a key the app simply shows the rule-based summary below.
    if ANTHROPIC_KEY:
    ...
    st.warning(
        "Market data is taking a moment to load. Try again, check your ticker symbols, "
        "or switch on Demo mode in the sidebar to use saved data."
    )
